 ...
 *
 */
GeneralElementOld::GeneralElementOld()
{
  normal_calculated = 0;
 ...
 *
 */
void GeneralElementOld::check_normal_direction(double *normal, double *out)
{
 ...
 *
 */
double *GeneralElementOld::get_centroid()
{
 ...
 *
 */
double *GeneralElementOld::get_normal()
{
 ...
 *
 */
void LineOld::init(double **p, double *o, int *c)
{
 ...
 *
 */
void LineOld::calculate_centroid()
{
 ...
 *
 */
PolygonOld::PolygonOld()
{
  GeneralElementOld();
  lines = new LineOld*[MAX_LINES];
}
 ...
 *
 */
bool PolygonOld::add_line(LineOld *l, int donotcheck)
{
  double eps = 1e-10;
 ...
 *
 */
void PolygonOld::calculate_centroid()
{
 ...
 *
 */
void PolygonOld::calculate_normal()
{
 ...
 *
 */
void PolygonOld::print(ofstream &file, double *pos)
{
  for (int i=0; i < Nlines; i++) {
    double *p1 = lines[i]->get_start();
    double *p2 = lines[i]->get_end();
 ...
 *
 */
PolyhedronOld::PolyhedronOld()
{
  GeneralElementOld();
  polygons = new PolygonOld*[MAX_POLYGONS];
}
 ...
 *
 */
bool PolyhedronOld::add_polygon(PolygonOld *p, int donocheck)
{
  ...
  if ( donocheck || Npolygons == 0 ) {
    ...
    for (int i=0; i < Npolygons; i++) {
      ...
      LineOld **lines1 = p->get_lines();
      LineOld **lines2 = polygons[i]->get_lines();
 ...
 *
 */
bool PolyhedronOld::lines_equal(LineOld *l1, LineOld *l2)
{
  double eps = 1e-10;
 ...
 *
 */
void PolyhedronOld::tetravolume(double *a, double *b, double *c, double *n)
{
 ...
 * 
 */
void PolyhedronOld::calculate_centroid()
{
  double *mean = new double[DIM];
  for (int i=0; i < DIM; i++ ) {
    mean[i] = 0;
  ...
  for (int i=0; i < Npolygons; i++ ) {
    ...
    LineOld **lines = polygons[i]->get_lines();
 ...
 *
 */
void PolyhedronOld::calculate_normal()
{
 ...
 *
 */
void SquareOld::init(double **sq, int *c_i, double *c_v, double *dex)
{
  for (int i=0; i < SQUARE_DIM; i++) {
 ...
 *
 */
void SquareOld::construct_lines(double E0)
{
 ...
 *
 */
void SquareOld::ends_of_edge(double E0)
{
  //Edge 1
 ...
 *
 */
void SquareOld::find_outside(double E0)
{
...
CubeOld::CubeOld()
{
  ...
  lines = new LineOld*[NSQUARES*2]; //Each square may have max. 2 lines
  polygons = new PolygonOld[MAX_POLY];
  squares = new SquareOld[NSQUARES];
}
 ...
 *
 */
void CubeOld::init(double ***&c, int c_i, double c_v, double *&dex)
{
  const_i = c_i;
  const_value = c_v;
  dx = dex;
 ...
 *
 */
void CubeOld::construct_polygons(double value0)
{
 ...
 *
 */
void CubeOld::check_ambiguous(int Nlines)
{
 ...
 *
 */
PolygonOld* CubeOld::get_polygons()
{
  return polygons;
}
...
HypercubeOld::HypercubeOld()
{
  ...
  polyhedrons = new PolyhedronOld[MAX_POLY];
  polygons = new PolygonOld*[NCUBES*10];
  cubes = new CubeOld[NCUBES];
}
 ...
 *
 */
void HypercubeOld::init(double ****c, double *dex)
{
  dx = dex;
  //Here we fix the non-zero indices
  x1 = 0;
 ...
 *
 */
void HypercubeOld::split_to_cubes()
{
 ...
 *
 */
void HypercubeOld::construct_polyhedrons(double value0)
{
  split_to_cubes();
  ...
  for (int i=0; i < NCUBES; i++) {
    ...
    PolygonOld *p = cubes[i].get_polygons();
 ...
 *
 */
void HypercubeOld::check_ambiguous(double value0)
{
  for (int i=0; i < NCUBES; i++) {
 ...
 *
 */
int HypercubeOld::get_Npolyhedrons()
{
  return Npolyhedrons;
}
 ...
 *
 */
PolyhedronOld* HypercubeOld::get_polyhedrons()
{
  return polyhedrons;
}
...
/**
 *
 * Initializes CorneliusOld. Can be used several times without any problems. This
 ...
 *
 */
void CorneliusOld::init(int d, double v0, double *dex)
{
  cube_dim = d;
  value0 = v0;
  if ( initialized != 1 ) {
 ...
 *
 */
void CorneliusOld::init_print(string filename)
{
 ...
 *
 */
void CorneliusOld::find_surface_2d(double **cube)
{
  if ( !initialized || cube_dim != 2 ) {
    cout << "CorneliusOld not initialized for 2D case" << endl;
    exit(1);
  }
 ...
 *
 */
void CorneliusOld::find_surface_3d(double ***cube)
{
  double *pos = NULL;
 ...
 *
 */
void CorneliusOld::find_surface_3d_print(double ***cube, double *pos)
{
 ...
 *
 */
void CorneliusOld::surface_3d(double ***cube, double *pos, int do_print)
{
  if ( !initialized || cube_dim != 3 ) {
    cout << "CorneliusOld not initialized for 3D case" << endl;
    exit(1);
  }
  ...
  PolygonOld *p = cu3d.get_polygons();
  for (int i=0; i < Nelements; i++) {
 ...
 *
 */
void CorneliusOld::find_surface_4d(double ****cube)
{
  if ( !initialized || cube_dim != 4 ) {
    cout << "CorneliusOld not initialized for 4D case" << endl;
    exit(1);
  }
  //First we check if the cube actually contains surface elements.
  //If all or none of the elements are below the criterion, no surface
  ...
  PolyhedronOld *p = cu4d.get_polyhedrons();
  for (int i=0; i < Nelements; i++) {
    for (int j=0; j < DIM; j++) {
      centroids[i][j] = p[i].get_centroid()[j];
      normals[i][j] = p[i].get_normal()[j];
 ...
 *
 */
int CorneliusOld::get_Nelements()
{
  return Nelements;
}
 ...
 *
 */
double** CorneliusOld::get_centroids_4d()
{
 ...
 *
 */
double** CorneliusOld::get_normals_4d()
{
  double **vect = new double*[Nelements];
  for (int i=0; i < Nelements; i++) {
    vect[i] = new double[DIM];
 ...
 *
 */
double** CorneliusOld::get_centroids()
{
  double **vect = new double*[Nelements];
  for (int i=0; i < Nelements; i++) {
 ...
 *
 */
double** CorneliusOld::get_normals()
{
  double **vect = new double*[Nelements];
  for (int i=0; i < Nelements; i++) {
    vect[i] = new double[cube_dim];
 ...
 *
 */
double CorneliusOld::get_centroid_elem(int i, int j)
{
  if ( i >= Nelements || j >= cube_dim ) {
    cout << "CorneliusOld error: asking for an element which does not exist." << endl;
    exit(1);
  }
 ...
 *
 */
double CorneliusOld::get_normal_elem(int i, int j)
{
  if ( i >= Nelements || j >= cube_dim ) {
    cout << "CorneliusOld error: asking for an element which does not exist." << endl;
    exit(1);
